...
class BooksController{
    static findAll = async (req, res) => {
        const list = await Book.find({}).populate(["author", "publisher"]).exec()
        res.json(list)
    }
    static findById = async (req, res) => {
        const {id} = req.params;
        try{
            const book = await Book.findById(id).populate(["author", "publisher"]).exec()
            res.json(book)
        }catch (e){
    ...
    static findByEditor = async (req, res) => {
        ...
        try {
            const books = await Book.find({editor: editor}).populate(["author", "publisher"]).exec();
            res.json(books)
        }catch (e) {
